...
Результат последнего запуска сохранён в репозитории:

```text
reports/fuzzing/schemathesis-result.txt
```

В ходе тестирования были получены ответы 403 Forbidden для защищённых эндпоинтов, требующих аутентификации. Такое поведение соответствует настройкам безопасности приложения.
